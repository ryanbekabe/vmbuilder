...
class Dapper(suite.Suite):
    ...
    def mount_dev_proc(self):
        ...
        run_cmd('mount', '-t', 'proc', 'proc', '%s/proc' % self.context.chroot_dir)
    ...
    def install_extras(self):
        ...
        cmd = ['apt-get', 'install', '-y', '--force-yes', '--option=Dpkg::Options::=--force-confnew']
        cmd += addpkg or []
    ...
    def install_kernel(self, destdir):
        self.mount_dev_proc()
        run_cmd('chroot', destdir, 'apt-get', '--force-yes', '--option=Dpkg::Options::=--force-confnew', '-y', 'install', self.kernel_name(), env={'DEBIAN_FRONTEND': 'noninteractive'})
